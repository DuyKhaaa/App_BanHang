    ...
    private void loadBookProducts() {
        try {
            ...
            Cursor cursor = database.rawQuery("SELECT ProductID, NamePro, Status, DecriptionPro, Image, NameCate " +
                    ...
                    "WHERE tbCategory.NameCate = 'Sổ tay lưu niệm'", null);
            ...
            if (cursor != null && cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    ...
                    bookProducts.add(new Product(productId, productName, status, imageData, categoryName, decriptionPro));
                }
                cursor.close();
            } else {
            ...
            productAdapter = new ProductAdapter(this, bookProducts, new ProductAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(int position) {
                    // Xử lý khi một item được nhấn
                    Product product = bookProducts.get(position);
                    ...
                    Intent intent = new Intent(BookActivity.this, ChitietspActivity.class);
