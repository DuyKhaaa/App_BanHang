...
public class MagnetActivity extends AppCompatActivity {
    ...
    private void loadMagnetProducts() {
        try {
            magnetProducts = new ArrayList<>();
            ...
            Cursor cursor = database.rawQuery("SELECT ProductID, NamePro, Status, DecriptionPro, Image, NameCate " +
                    ...
                    "WHERE tbCategory.NameCate = 'Magnet nam châm'", null);
            ...
            if (cursor != null && cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    ...
                    magnetProducts.add(new Product(productId, productName, status, imageData, categoryName, decriptionPro));
                }
                cursor.close();
            } else {
            ...
            productAdapter = new ProductAdapter(this, magnetProducts, new ProductAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(int position) {
                    // Xử lý khi một item được nhấn
                    Product product = magnetProducts.get(position);
                    ...
                    Intent intent = new Intent(MagnetActivity.this, ChitietspActivity.class);
    ...
    private void addControls() {
        recyclerView = findViewById(R.id.recycler_view_magnet);
