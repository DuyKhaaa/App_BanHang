...
public class BookMarkActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addControls();
        // Thực hiện truy vấn SQLite để lấy các sản phẩm có category là "bookmark"
        loadBookMarkProducts();
        sessionManager = new SessionManager(this);
        addEvents();
    }
    private void loadBookMarkProducts() {
        // Thực hiện truy vấn SQLite để lấy dữ liệu sản phẩm từ bảng tbProducts với điều kiện là chỉ lấy các sản phẩm có category là "bookmark"
        try {
            ...
            Cursor cursor = database.rawQuery("SELECT ProductID, NamePro, Status, DecriptionPro, Image, NameCate " +
                    ...
                    "WHERE tbCategory.NameCate = 'Bookmark'", null);
            ...
            if (cursor != null && cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    ...
                    bookmarkProducts.add(new Product(productId, productName, status, imageData, categoryName, decriptionPro));
                }
                cursor.close();
            } else {
            ...
            productAdapter = new ProductAdapter(this, bookmarkProducts, new ProductAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(int position) {
                    // Xử lý khi một item được nhấn
                    Product product = bookmarkProducts.get(position);
                    ...
                    Intent intent = new Intent(BookMarkActivity.this, ChitietspActivity.class);
