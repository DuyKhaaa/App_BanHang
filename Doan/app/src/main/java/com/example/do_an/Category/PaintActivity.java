...
public class PaintActivity extends AppCompatActivity {
    ...
    private void loadPaintProducts() {
        try {
            paintProducts = new ArrayList<>();
            ...
            Cursor cursor = database.rawQuery("SELECT ProductID, NamePro, Status, DecriptionPro, Image, NameCate " +
                    ...
                    "WHERE tbCategory.NameCate = 'Sơn mài mỹ nghệ'", null);
            ...
            if (cursor != null && cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    ...
                    paintProducts.add(new Product(productId, productName, status, imageData, categoryName, decriptionPro));
                }
                cursor.close();
            } else {
            ...
            productAdapter = new ProductAdapter(this, paintProducts, new ProductAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(int position) {
                    // Xử lý khi một item được nhấn
                    Product product = paintProducts.get(position);
                    ...
                    Intent intent = new Intent(PaintActivity.this, ChitietspActivity.class);
    ...
    private void addControls() {
        recyclerView = findViewById(R.id.recycler_view_paint);
