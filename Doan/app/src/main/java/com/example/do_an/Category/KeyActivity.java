...
public class KeyActivity extends AppCompatActivity {
    ...
    private void loadKeyProducts() {
        try {
            ...
            Cursor cursor = database.rawQuery("SELECT ProductID, NamePro, Status, DecriptionPro, Image, NameCate " +
                    ...
                    "WHERE tbCategory.NameCate = 'Móc khóa'", null);
            ...
            if (cursor != null && cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    ...
                    keyProducts.add(new Product(productId, productName, status, imageData, categoryName, decriptionPro));
                }
                cursor.close();
            } else {
            ...
            productAdapter = new ProductAdapter(this, keyProducts, new ProductAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(int position) {
                    // Xử lý khi một item được nhấn
                    Product product = keyProducts.get(position);
                    ...
                    Intent intent = new Intent(KeyActivity.this, ChitietspActivity.class);
    ...
    private void addControls() {
        recyclerView = findViewById(R.id.recycler_view_key);
